...
def upload_thumbnail(wp, image_url, filename):
    """画像URLをWPメディアライブラリにアップロードしてIDを返す。失敗時はNone。"""
    try:
        resp = requests.get(image_url, timeout=10)
        if resp.status_code != 200:
            return None
        result = wp.call(UploadFile({
            'name': filename,
            'type': 'image/jpeg',
...
def wp_post(title, content, slug, category, tags, seo_title, seo_desc, thumbnail_url=None):
    ...
    if slug in existing:
        ...
        if thumbnail_url and not p.thumbnail:
            media_id = upload_thumbnail(wp, thumbnail_url, f"{slug}.jpg")
            if media_id:
                p.thumbnail = media_id
...
"""
    # 1位作品のラージ画像をアイキャッチに使用
    thumbnail_url = (items[0].get('imageURL') or {}).get('large') or \
                    (items[0].get('imageURL') or {}).get('small')
